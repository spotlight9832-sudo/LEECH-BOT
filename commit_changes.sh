...
# 2. bot/helper/themes/__init__.py
cat << 'PY_EOF' > bot/helper/themes/__init__.py
class KPSML_THEME:
    LEECH_SET_M = '''〔 LEECH CONTROL PANEL 〕\n\n● Usage: Unlimited [ {dailyleech} ]\n● Type: {leech_type}\n● Split Size: {split_size}\n\n--- CONFIGURATION ---\n» Thumbnail: {thumb}\n» Equal Splits: {equal_splits}\n» Media Group: {media_group}\n» Stream mod: {stream_mod}\n» Screenshots: {screenshots}\n» Font Style: {font_style}\n\n--- CUSTOM TEXT ---\n» Prefix: {l_pref}\n» Suffix: {l_suf}\n» Metadata: {l_meta}\n'''

    STATUS_LIMIT = '''\n┌─────────────────────────┐\n     ◈ ＳＹＳＴＥＭ  ＴＲＡＮＳＦＥＲ ◈\n└─────────────────────────┘\n\n  ➥  𝗢𝗯𝗷𝗲𝗰𝘁   ﹕ [{task_name}]\n\n  ⌽  𝗣𝗿𝗼𝗴𝗿𝗲𝘀𝘀 ﹕ {progress}%           ⌽\n  ┕─ [ {bar} ] ─┙\n  ⊶  ꜱᴛᴀᴛᴜꜱ    ﹕ {status}\n  ⊶  ᴅᴀᴛᴀ      ﹕ {processed} / {size} (⏳ {eta})\n  ⊶  ᴠᴇʟᴏᴄɪᴛʏ  ﹕ {speed}\n  ⊶  ᴍᴏᴅᴇ      ﹕ {engine}\n╼─────────────────────────────────────╾\n  👤  𝗜𝗱𝗲𝗻𝘁𝗶𝘁𝘆 ﹕ {mention} | ID: {uid} ⊘ 𝗔𝗰𝘁𝗶𝗼𝗻 ﹕ {cancel}\n╼─────────────────────────────────────╾\n  ≡  𝗦𝗧𝗔𝗧𝗦﹕≡ 𝗦𝗬𝗦﹕𝖢: {cpu}% | 𝖥: {free}\n  𝖱: {ram}% | 𝖴𝗉: {uptime}\n  ≡  𝗡𝗘𝗧   ﹕ ↓ {dl_speed} | ↑ {ul_speed} ╼────────────────────────────────────╾\n'''
    STATUS_OBJ = STATUS_LIMIT

    CLONE_END_MSG = '''\n<a href=\"https://t.me/SECRECT_BOT_UPDATES\">ꜱʏꜱᴛᴇᴍ ᴏᴜᴛᴘᴜᴛ: ꜰɪʟᴇ ᴛʀᴀɴꜱꜰᴇʀ</a>\n━━━━━━━━━━━━━━━━━━━━━━━━━━\nᴛɪᴛʟᴇ : {Name}\n┃ sɪᴢᴇ : {size}\n┃ ᴛɪᴍᴇ : {elapsed}\n┃ ᴍᴏᴅᴇ :  #Leech | #Tg\n┃ ᴛᴏᴛᴀʟ ꜰɪʟᴇs: {total_files}\n┃ ᴜᴘʟᴏᴀᴅᴇʀ : {mention}\nɴᴏᴛɪғɪᴄᴀᴛɪᴏɴ\nᴛʜᴇ ғɪʟᴇ ʜᴀs ʙᴇᴇɴ sᴜᴄᴄᴇssғᴜʟʟʏ ʀᴏᴜᴛᴇᴅ\nᴛᴏ ʏᴏᴜʀ ᴘʀɪᴠᴀᴛᴇ ᴍᴇssᴀɢᴇs (ʙᴏᴛ ᴘᴍ).\n━━━━━━━━━━━━━━━━━━━━━━━━━━\nɴᴇᴛᴡᴏʀᴋ: @SECRECT_BOT_UPDATES\n1. {Name}\n{stream_link}{download_link}'''
    LEECH_END_MSG = CLONE_END_MSG
    MIRROR_END_MSG = CLONE_END_MSG

PY_EOF

# 3. bot/modules/bot_settings.py
cat << 'PY_EOF' > fix_bot_settings.py
import re

filepath = "bot/modules/bot_settings.py"
with open(filepath, "r", encoding="utf-8") as f:
    content = f.read()

if "VERCEL_URL" not in content:
    content = content.replace("config_dict['BASE_URL'] = environ.get('BASE_URL', '')", "config_dict['BASE_URL'] = environ.get('BASE_URL', '')\n    config_dict['VERCEL_URL'] = environ.get('VERCEL_URL', '')\n    config_dict['VERCEL_API'] = environ.get('VERCEL_API', '')")

with open(filepath, "w", encoding="utf-8") as f:
    f.write(content)
PY_EOF
python3 fix_bot_settings.py
rm fix_bot_settings.py

# 4. bot/modules/users_settings.py
cat << 'PY_EOF' > fix_users_settings.py
import re

filepath = "bot/modules/users_settings.py"
with open(filepath, "r", encoding="utf-8") as f:
    content = f.read()

new_func = r'''    text = (
        theme.LEECH_SET_M.format(
            mention=message.from_user.mention,
            dailyleech=dailyleech,
            leech_type=leech_type,
            thumb=thumb,
            split_size=split_size,
            equal_splits=equal_splits,
            media_group=media_group,
            stream_mod="✅ Enabled" if user_dict.get('stream_mod') else "✘ Disabled",
            screenshots="✅ Enabled" if user_dict.get('screenshots') else "✘ Disabled",
            font_style=user_dict.get('font_style', 'Bold Caption'),
            l_cap=l_cap,
            l_pref=l_pref,
            l_suf=l_suf,
            l_rem=l_rem,
...
content = re.sub(r"text = \(\s*theme\.LEECH_SET_M\.format\([\s\S]*?l_meta=l_meta\s*\)\s*\)", new_func, content)

with open(filepath, "w", encoding="utf-8") as f:
    f.write(content)
PY_EOF
python3 fix_users_settings.py
rm fix_users_settings.py

# 5. config_sample.env
cat << 'PY_EOF' >> config_sample.env
VERCEL_URL = "https://vercel-steam-mod.vercel.app"
VERCEL_API = "sk_live_vrc_9s8f7a6h5g4f3d2s1a"
PY_EOF
