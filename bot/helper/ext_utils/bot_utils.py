...
from re import match as re_match, search as re_search, IGNORECASE
...
def get_readable_message():
    ...
    for download in list(download_dict.values())[STATUS_START:STATUS_LIMIT+STATUS_START]:
        ...
        if config_dict['SAFE_MODE'] and elapsed >= config_dict['STATUS_UPDATE_INTERVAL']:
            obj_name = "Task is being Processed!"
            series_name = ""
        else:
            name = download.name()
            match = re_search(r'(.*?)\s((?:S\d+|EP?\d+|Complete|Season|Year).*)$', name, IGNORECASE)
            if match:
                obj_name = escape(match.group(1))
                series_name = escape(match.group(2))
            else:
                obj_name = escape(name)
                series_name = ""
        ...
        if download.status() not in [MirrorStatus.STATUS_SPLITTING, MirrorStatus.STATUS_SEEDING]:
            msg += BotTheme('PROGRESS', Progress=download.progress())
            msg += BotTheme('BAR', Bar=get_progress_bar_string(download.progress()))
